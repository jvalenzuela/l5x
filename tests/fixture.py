...
def create_tag_element(datatype, name='TestTag', data=1, dim=()):
    """Creates a mock XML element defining a top-level tag."""
    ...
    data_element = ElementTree.SubElement(element, 'Data')
    data_element.text = bytearray(data).hex(' ')

    return element


def create_member_element(datatype, name='TestMember', dim=(0,)):
    """Creates a mock XML element defining a structure member."""
